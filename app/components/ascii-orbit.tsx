...
function renderFrame(points: Point[], time: number, seed: number) {
  ...
  points.forEach((point, index) => {
    ...
    const x = Math.round(width / 2 + rotated.x * perspective * 17);
    const y = Math.round(height / 2 + rotated.y * perspective * 8.5);
...
export function AsciiOrbit() {
  ...
  return (
    <div className="flex w-full justify-center">
      <pre
        aria-hidden="true"
        className="w-fit max-w-full overflow-hidden rounded-md border border-neutral-200 bg-neutral-50 px-3 py-4 text-[9px] leading-[1.2] text-neutral-700 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-300 sm:px-4 sm:text-xs"
      >
        {frame}
      </pre>
    </div>
